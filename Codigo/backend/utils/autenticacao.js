...
const checarPermissao = require("./checarPermissao");
...
const autenticacao = (requerAutenticacao, permissao) => (req, res, next) => {
  ...
  if (requerAutenticacao) {
    ...
    jwt.verify(token, process.env.SECRET, async (err, decoded) => {
      ...
      if (!permissao || req.perfil == "admin" || (await checarPermissao(req.perfil,permissao) && req.status == 1)) {
 
        next();
      } else {
        return res
          .status(401)
          .json({ error: "Usuário inativo ou não possui permissão de acesso" });
      }
    });
  } else {
    next();
